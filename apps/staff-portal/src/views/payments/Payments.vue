...
export default {
  ...
  methods: {
    ...
    async loadFilterOptions() {
      try {
        const [paymentsResponse, programsResponse, academicSessionsResponse] = await Promise.all([
          apiService.getPayments({ page: 1, limit: 500, active: true, sortBy: 'name', sortOrder: 'asc' }),
    ...
    async loadPayments() {
      try {
        ...
        if (!response.success) {
          throw new Error(response.message || 'Failed to load student payments')
        }
    ...
    getProgramDisplay(payment) {
      return [payment.programTypeLabel, payment.programModeLabel, payment.programName]
        .filter(value => value && value !== 'N/A')
        .join(' ') || 'N/A'
    },

    getProgramOptionLabel(program) {
      const type = program?.programType?.type || program?.programType || program?.programTypeLabel
      const mode = program?.programMode?.mode || program?.programMode || program?.programModeLabel
      return [type, mode, program?.name].filter(Boolean).join(' ') || program?.name || 'N/A'
    },
    ...
    canReviewManualTransfer(payment) {
      return this.isManualTransferPayment(payment) && payment.status === 'pending' && !!payment.receiptUrl
    },
    ...
    getReceiptExtension(payment) {
      const source = (payment?.receiptOriginalName || payment?.receiptUrl || '').split('?')[0]
    ...
    async verifyManualTransferPayment(payment) {
      try {
        ...
        const response = await apiService.verifyManualTransferPayment(payment._id, {
    ...
    async rejectManualTransferPayment(payment) {
      try {
        ...
        const response = await apiService.rejectManualTransferPayment(payment._id, {
      ...
      <div class="col-12">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
          <div>
            ...
            <p class="text-muted mb-0">Search, filter, and manage student payment records across academic sessions and programmes.</p>
          </div>
          <div class="d-flex flex-wrap gap-2">
            <button class="btn btn-outline-staff-primary btn-sm" @click="resetFilters">
            ...
            <div class="row g-3">
              ...
              <div class="col-xl-2 col-md-4 col-sm-6">
                ...
                <select v-model="filters.method" class="form-select">
                  <option v-for="option in methodOptions" :key="option.value || 'all-methods'" :value="option.value">
                    {{ option.label }}
                  </option>
                </select>
              </div>
              <div class="col-xl-2 col-md-6 col-sm-6">
                ...
                <select v-model="filters.paymentId" class="form-select">
                  ...
                  <option v-for="payment in paymentOptions" :key="payment.id" :value="payment.id">
                    {{ payment.name }}
                  </option>
                </select>
              </div>
            ...
            <div class="row g-3 mt-1">
              ...
              <div class="col-lg-3 col-md-4 d-flex align-items-end">
                <div class="filter-summary-panel w-100">
                  <div class="small text-muted text-uppercase fw-semibold">Records</div>
                  <div class="fw-bold text-staff-primary fs-5">{{ totalPayments }}</div>
                  <small class="text-muted">{{ hasActiveFilters ? 'Matching current filters' : 'All available payment records' }}</small>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card-body p-0">
            <div v-if="payments.length === 0" class="text-center py-5">
              <div class="text-muted px-3">
                ...
                <p class="mb-0">
                  {{ hasActiveFilters ? 'No student payments match your current filters.' : 'Student payment records will appear here once payments are created.' }}
                </p>
              </div>
            </div>
            ...
            <template v-else>
                ...
                <table class="table table-hover mb-0 align-middle payments-table">
                  ...
                  <tbody>
                    <tr v-for="payment in payments" :key="payment._id">
                      <td class="user-cell">
                        <div class="fw-semibold">{{ getUserDisplayName(payment) }}</div>
                        <div class="small text-muted">
                          <!-- {{ getIdentifierLabel(payment) }}:  -->
                          {{ getIdentifierValue(payment) }}
                        </div>
                        <div class="small text-muted text-break d-none d-xxl-block">{{ payment.email || 'No email available' }}</div>
                      </td> 
                      <td class="program-cell">
                        <div class="fw-medium">{{ getProgramDisplay(payment) }}</div>
                      </td>
                      <td>
                        <span class="badge bg-light text-dark border">{{ payment.academicSessionLabel || 'N/A' }}</span>
                      </td>
                      <td>
                        <div class="fw-semibold">{{ payment.paymentName || 'N/A' }}</div>
                        <code class="payment-reference mt-1">{{ getReferenceDisplay(payment.reference) }}</code>
                      </td>
                      <td>
                        <div class="fw-semibold">{{ formatCurrency(payment.amount) }}</div>
                      </td>
                      <td>
                        <span class="badge rounded-pill" :class="getMethodBadgeClass(payment.method)">
                          {{ formatLabel(payment.method) }}
                        </span>
                      </td>
                      <td>
                        <span class="badge rounded-pill" :class="getStatusBadgeClass(payment.status)">
                          {{ formatLabel(payment.status) }}
                        </span>
                      </td>
                      <td>
                        <div>{{ formatDateTime(getEffectivePaidDate(payment)) }}</div>
                        <div v-if="!payment.paidAt && payment.createdAt" class="small text-muted">Initiated record</div>
                      </td>
                      <td>
                        <div class="d-flex flex-wrap gap-2 actions-cell">
                          <button
                          ...
                          <template v-if="canReviewManualTransfer(payment)">
                            ...
                            >
                              <span v-if="isProcessingPayment(payment._id)" class="spinner-border spinner-border-sm me-1"></span>
              ...
              <div class="d-lg-none p-3">
                <div class="row g-3">
                  <div v-for="payment in payments" :key="`mobile-${payment._id}`" class="col-12">
                    <div class="payment-mobile-card h-100">
                      <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
                        <div>
                          <div class="fw-semibold text-staff-primary">{{ getUserDisplayName(payment) }}</div>
                          <div class="small text-muted">
                            <!-- {{ getIdentifierLabel(payment) }}:  -->
                            {{ getIdentifierValue(payment) }}
                          </div>
                        ...
                        <div class="text-end">
                          <div class="fw-semibold">{{ formatCurrency(payment.amount) }}</div>
                          <small class="text-muted">{{ formatDateTime(getEffectivePaidDate(payment)) }}</small>
                        </div>
                      </div>
                      ...
                      <div class="payment-mobile-meta d-grid gap-2 mb-3">
                        ...
                        <div class="row g-2">
                          <div class="col-12 col-sm-6">
                            <div class="small text-uppercase text-muted fw-semibold">Session</div>
                            <div>{{ payment.academicSessionLabel || 'N/A' }}</div>
                          </div>
                          <div class="col-12 col-sm-6">
                            <div class="small text-uppercase text-muted fw-semibold">Payment</div>
                            <div>{{ payment.paymentName || 'N/A' }}</div>
                          </div>
                        </div>
                        <div>
                          <div class="small text-uppercase text-muted fw-semibold">Reference</div>
                          <code class="payment-reference mt-1">{{ getReferenceDisplay(payment.reference) }}</code>
                        </div>
                      </div>
                      ...
                      <div class="d-flex flex-wrap gap-2 mb-3">
                        ...
                        <span v-if="!payment.paidAt && payment.createdAt" class="badge bg-light text-dark border">
                          Initiated record
                        </span>
                      </div>
            ...
            <nav>
              <ul class="pagination pagination-sm mb-0 justify-content-center flex-wrap gap-1">
                <li class="page-item" :class="{ disabled: currentPage === 1 }">
                  <button class="page-link" :disabled="currentPage === 1" @click="currentPage = currentPage - 1">
                    Previous
                  </button>
                </li>
                <li v-for="page in apiTotalPages" :key="page" class="page-item" :class="{ active: currentPage === page }">
                  <button class="page-link" @click="currentPage = page">{{ page }}</button>
                </li>
                <li class="page-item" :class="{ disabled: currentPage >= apiTotalPages }">
                  <button class="page-link" :disabled="currentPage >= apiTotalPages" @click="currentPage = currentPage + 1">
                    Next
                  </button>
                </li>
              </ul>
